...
/// <summary>
/// 提供一个资源池，支持重用派生自 <see cref="FontIconSource"/> 的 <typeparamref name="T"/> 类型的实例
/// </summary>
/// <typeparam name="T">资源池中对象的类型</typeparam>
public sealed class FontIconSourcePool<T> where T : FontIconSource, new()
{
    /// <summary>
    /// 懒加载的资源池实例容器，仅在首次访问时会该创建资源池实例
    /// </summary>
    private static readonly Lazy<FontIconSourcePool<T>> s_lazy = new(() => new());

    /// <summary>
    /// 获取共享的 <see cref="FontIconSourcePool{T}"/> 的实例
    /// </summary>
    public static FontIconSourcePool<T> Shared => s_lazy.Value;
    ...
    /// <summary>
    /// 初始化 <see cref="FontIconSourcePool{T}"/> 的静态成员
    /// </summary>
    private FontIconSourcePool()
    {
