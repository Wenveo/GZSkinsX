...
internal sealed class MyModsService(MyModsSettings myModSettings) : IMyModsService
{
    ...
    public async Task<BitmapImage> GetModImageAsync(StorageFile storageFile)
    {
        ...
        try
        {
            ...
            await MyModsHelper.ExtractModImageAsync(storageFile.Path, imageFilePath);
    ...
    {
        return await MyModsHelper.ReadModInfoAsync(storageFile.Path);
    }
    ...
    public async Task<MyModInfo?> TryReadModInfoAsync(StorageFile storageFile)
    {
        ...
        try
        {
            result = await MyModsHelper.ReadModInfoAsync(storageFile.Path);
        }
        catch (Exception excp)
        {
            AppxContext.LoggingService.LogError(
    ...
    private static async Task<string> TryGetSettingsFilePathAsync()
    {
    ...
    private async Task<MTSettingsData?> TryGetSettingsDataAsync()
    {
        ...
        if (settingsRoot is null || settingsRoot.Data is null)
        {
            return null;
        }
    ...
    private async IAsyncEnumerable<string> TryParseRGZFileTableAsync(object fileTable)
    {
        ...
        {
            str = fileTable is string s ? s : fileTable.ToString() ?? string.Empty;
        }
        ...
        var result = await MyModsHelper.DecryptConfigTextAsync(str);
...
#pragma warning disable format
        ...
        settingsData.GZSkins["RGZFileTable"]    = await MyModsHelper.EncryptConfigTextAsync(stringBuilder.ToString());
#pragma warning restore format

        var parentFolder = Path.GetDirectoryName(settingsFilePath);
        if (!string.IsNullOrEmpty(parentFolder) &&
            Directory.Exists(parentFolder) is false)
        {
