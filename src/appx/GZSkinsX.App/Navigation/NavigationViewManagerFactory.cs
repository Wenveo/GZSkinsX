...
internal sealed class NavigationViewManagerFactory : INavigationViewManagerFactory
{
    ...
    /// <summary>
    /// 通过解析 <see cref="NavigationItemContext"/> 上下文对象并创建 <see cref="NavigationViewItem"/> 对象实例
    /// </summary>
    /// <param name="context">需要解析的 <see cref="NavigationItemContext"/> 上下文对象</param>
    /// <returns>已创建的 <see cref="NavigationViewItem"/> 对象实例</returns>
    private NavigationViewItem CreateNavItemUIObject(NavigationItemContext context)
    {
        var guid = new Guid(context.Metadata.Guid!);
        var navItem = new NavigationViewItem { DataContext = context, Tag = guid };
    ...
    private NavigationViewManager CoreceNavigationViewManager(string ownerGuidString, NavigationView navigationView)
    {
    ...
    => CoreceNavigationViewManager(ownerGuidString, new NavigationView());
