...
internal sealed class WadFile : IDisposable
{
    ...
    private readonly Dictionary<ulong, WadFileChunk> _fileChunks;
    private readonly Stream _stream;
    private bool _isDisposed;

    public IReadOnlyDictionary<ulong, WadFileChunk> FileChunks => _fileChunks;
    ...
    public WadFile(Stream inputStream)
    {
        ...
        for (var i = 0; i < chunkCount; i++)
        {
            ...
            _fileChunks.Add(chunk.XxHash, chunk);
        }

        if (byt_major > 2)
        {
            var expectedChunkCount = _fileChunks.Values.Select(
                item => item.FirstChunkIndex + item.SubChunkCount).Max();

            var possibleChoice = _fileChunks.Values.FirstOrDefault(
    ...
    {
        return $"Version = v{FileVersion.Major}.{FileVersion.Minor}, FileCount = {_fileChunks.Count}";
    }
